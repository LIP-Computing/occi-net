...
class TestParser(base.TestCase):
    ...
    def test_param_from_headers(self):
        # TODO(jorgesece): the fake driver
        # should be improved to make parametriced query tests
        tenant_id = "33"
        headers = {
        ...
        parsed = parse_header.HeaderParser(headers, None).parse()
    ...
    def test_param_from_headers_mixin(self):
        ...
        headers = {
            'Category': '%s; scheme="%s";class="kind",' %
                        (network_term, network_scheme) +
                        '%s; scheme="%s"; class=mixin' %
                        (mixin_id, mixin_scheme),
            'X-OCCI-Attribute': 'tenant_id=%s, network_id=1' % tenant_id,
        }
        parameters = parse_header.HeaderParser(headers, None).parse()
        ...
        self.assertEqual('%s%s' % (network_scheme, network_term),
                         parameters["category"])
